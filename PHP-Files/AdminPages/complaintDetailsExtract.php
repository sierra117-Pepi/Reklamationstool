    ...
    function fillPageInforForComplaint($complaintNr){   
        ...
        if(mysqli_connect_errno()){
           header("Location:../ErrorPages/dbConnectionError.php");
            ...
            $query = "SELECT c.nr, c.status, c.type, c.description, c.reasonSchachinger, c.measureSchachinger, c.measureAvoid, p.name, p.details FROM complaints c JOIN products p ON c.nr = p.nr WHERE c.nr=?";
            ...
            if(!mysqli_stmt_execute($stmt)){
                ...
                $complaintInfo = array();
                mysqli_stmt_bind_result($stmt, $nr, $status, $type, $description, $reasonSchachinger, $measureSchachinger, $measureAvoid, $name, $details);
